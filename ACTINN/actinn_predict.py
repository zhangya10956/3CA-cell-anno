...
def get_parser(parser=None):
    ...
    parser.add_argument("-trs", "--train_set", type=str, help="Training set file path.", default='D:/PyCharm Community Edition 2021.2.3/dataset/DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1218-train-csv/1220-1218-trainONLYadata10lei-T.h5')
    parser.add_argument("-trl", "--train_label", type=str, help="Training label file path.", default='D:/PyCharm Community Edition 2021.2.3/dataset/DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1218-train-csv/1218-trainONLY10lei-Label.csv')
    parser.add_argument("-ts", "--test_set", type=str, help="Training set file path.", default='D:/PyCharm Community Edition 2021.2.3/dataset/DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1014-testadata10lei-T.h5')
    parser.add_argument("-lr", "--learning_rate", type=float, help="Learning rate (default: 0.0001)", default=0.0001)   #DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1218-train-csv/trainONLYadata10lei.h5
    parser.add_argument("-ne", "--num_epochs", type=int, help="Number of epochs (default: 50)", default=50)     #DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1218-train-csv/1218-trainONLY10lei-Label.csv
    parser.add_argument("-ms", "--minibatch_size", type=int, help="Minibatch size (default: 128)", default=128)     #DBSY/NeuCA-R/1-68K_PBMC/0719-HVG2000-noscale/CSV-1013/1014-testadata10lei.h5
    parser.add_argument("-pc", "--print_cost", type=bool, help="Print cost when training (default: True)", default=True)
    parser.add_argument("-o", "--outname", type=str, help="output file name.", default='./PBMC68K-10lei-ACTINN-predict_result.csv')      #PBMC68K-10lei-ACTINN-predict_result.csv
...
def initialize_parameters(nf, ln1, ln2, ln3, nt):
    ...
    W1 = tf.compat.v1.get_variable("W1", [ln1, nf], initializer = slim.xavier_initializer(seed = 3))    #原来是tf.contrib.layers.
    b1 = tf.compat.v1.get_variable("b1", [ln1, 1], initializer = tf.zeros_initializer())
    W2 = tf.compat.v1.get_variable("W2", [ln2, ln1], initializer = slim.xavier_initializer(seed = 3))
    b2 = tf.compat.v1.get_variable("b2", [ln2, 1], initializer = tf.zeros_initializer())
    W3 = tf.compat.v1.get_variable("W3", [ln3, ln2], initializer = slim.xavier_initializer(seed = 3))
    b3 = tf.compat.v1.get_variable("b3", [ln3, 1], initializer = tf.zeros_initializer())
    W4 = tf.compat.v1.get_variable("W4", [nt, ln3], initializer = slim.xavier_initializer(seed = 3))
...
if __name__ == '__main__':
    ...
    nt = len(set(train_label.iloc[:,0]))     #原来是train_label.iloc[:,1]
    train_set, test_set = scale_sets([train_set, test_set])
    type_to_label_dict = type_to_label_dict(train_label.iloc[:,0])    #原来是train_label.iloc[:,1]
    ...
    train_label = convert_type_to_label(train_label.iloc[:,0], type_to_label_dict)      #原来是train_label.iloc[:,1]
    train_label = one_hot_matrix(train_label, nt)
    parameters = model(train_set, train_label, test_set, args.learning_rate, args.num_epochs, args.minibatch_size, args.print_cost)
    ...
    if args.output_probability:
        ...
        test_predict.to_csv("./predicted_probabilities_PBMC68K.txt", sep="\t")    #原来路径上没有./
    ...
    predicted_label.to_csv(args.outname, sep=",", index=False)       #原来的是sep="\t"
